...
// Importar os novos componentes das abas administrativas
...
export default function PainelAdmin() {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(false); // Inicia fechado para mobile
  ...
  useEffect(() => {
    ...
    if (!adminLogado) {
      router.push("/login-medico"); // Redireciona para login médico (ou crie um login-admin)
    } else {
        ...
        `}>
          ...
          <nav className="flex flex-col gap-2 mt-6">
            {['dashboard','medicos','usuarios','configuracoes'].map(tab => (
              <SidebarLink
                key={tab}
                icon={`/icons/${tab}.svg`} // Verifique se os ícones existem (dashboard.svg, medicos.svg, usuarios.svg, configuracoes.svg)
          ...
          <button
            onClick={() => {
              ...
              router.push("/login-medico"); // Redireciona para o login. Crie um login-admin se for diferente.
            }}
